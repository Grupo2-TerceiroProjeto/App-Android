...
import com.example.gestok.domain.model.product.IngredienteComIdSpoonacular
import com.example.gestok.domain.model.product.IngredienteComNutrientes
import com.example.gestok.domain.model.product.IngredienteTraduzido
...
import com.example.gestok.domain.model.product.IngredientsProduct
import com.example.gestok.domain.model.product.IngredientsRecipe
import com.example.gestok.domain.model.product.Nutriente
...
class ProductApiViewModel(
    ...
    private val lingva: LingvaRepository,
    private val spoonacular: SpoonacularRepository,
    ...
    override fun salvarProduto(
        ...
        viewModelScope.launch {
            try {
                ...
                val produtoFormatado = ProductCreateModel(
                    nome = produto.nome,
    ...
    override fun atualizarProducao(produto: ProductModel) {
        viewModelScope.launch {
            try {
                isUpdatingMap[produto.id] = true
                val produtoAtualizado = ProductEditModel(
                    nome = produto.nome,
    ...
    override fun atualizarEstoque(produtos: List<ProductModel>, onBack: () -> Unit, onSucess: () -> Unit) {
        var houveErro = false
    ...
    override fun getReceita(produto: ProductModel, onResult: (List<IngredientsRecipe>) -> Unit) {
        viewModelScope.launch {
            try {
                delay(1000)
    ...
    override fun editarProduto(
        ...
        viewModelScope.launch {
            try {
                ...
                val produtoFormatado = ProductEditModel(
                    nome = produto.nome,
                    categoria = produto.categoria,
                    preco = produto.preco,
                    quantidade = produto.quantidade,
