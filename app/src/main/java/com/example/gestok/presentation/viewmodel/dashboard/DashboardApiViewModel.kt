...
import com.example.gestok.domain.model.dashboard.AssessmentModel
...
class DashboardApiViewModel(private val api: DashboardRepository, override val sessaoUsuario : UserSession) : DashboardViewModel(sessaoUsuario) {
    ...
    override fun getBuscarPedidosProximos7Dias(): List<OrderModel> {
    ...
    override fun getMediaAvaliacao() {
        ...
        var avaliacoes = mutableStateListOf<AssessmentModel>()
        ...
        viewModelScope.launch {
            try {
                ...
                if (!corpo.isNullOrBlank() && corpo != "[]") {
                    val gson = Gson()
                    val lista: List<AssessmentModel> = gson.fromJson(
                        corpo,
                        object : TypeToken<List<AssessmentModel>>() {}.type
                    )
